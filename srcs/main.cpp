...
int wait_client(int server_socket, int server_socket2)
{
    struct pollfd pollfds[MAX_CLIENTS + 2];
    pollfds[0].fd = server_socket;
    pollfds[0].events = POLLIN | POLLPRI;
	pollfds[1].fd = server_socket2;
    ...
    {
        // printf("useClient => %d\n", useClient);
        int pollResult = poll(pollfds, useClient + 2, 5000);
        if (pollResult > 0)
        {
			...
			if (pollfds[1].revents & POLLIN)
            {
                struct sockaddr_in cliaddr;
                socklen_t addrlen = sizeof(cliaddr);
                int client_socket = accept(server_socket2, (struct sockaddr *)&cliaddr, &addrlen);
                printf("accept success %s\n", inet_ntoa(cliaddr.sin_addr));
                for (int i = 2; i < MAX_CLIENTS; i++)
                {
                    if (pollfds[i].fd == 0)
...
int main()
{
    int server_socket = creat_socket(9000);
    int server_socket2 = creat_socket(9001);

    int client_socket = wait_client(server_socket, server_socket2);
