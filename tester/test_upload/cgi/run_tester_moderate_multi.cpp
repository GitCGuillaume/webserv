...
{
	std::ifstream file("/mnt/nfs/homes/gchopin/Documents/webserv/tester/test_upload/files/1mo", std::ifstream::in | std::ifstream::binary);
	std::ifstream file2("/mnt/nfs/homes/gchopin/Documents/webserv/tester/test_upload/files/1mo2", std::ifstream::in | std::ifstream::binary);
	...
	header.insert(header.size(), "\r\n--myboundary\r\n");											// 16
	header.insert(header.size(),
				  "Content-Disposition: form-data; name=\"upload_files[]\"; filename=\"1mo2\"\r\n"); // 72
	header.insert(header.size(), "Content-Type: application/octet-stream\r\n\r\n");					 // 42
	header.insert(header.size(), buf2);																 // 1048576
	header.insert(header.size(), "\r\n--myboundary\r\n");											 // 16
	header.insert(header.size(),
				  "Content-Disposition: form-data; name=\"upload_files[]\"; filename=\"1mo3\"\r\n"); // 72
	header.insert(header.size(), "Content-Type: application/octet-stream\r\n\r\n");					 // 42
	header.insert(header.size(), buf2);																 // 1048576
	header.insert(header.size(), "\r\n--myboundary\r\n");											 // 16
	header.insert(header.size(),
				  "Content-Disposition: form-data; name=\"upload_files[]\"; filename=\"1mo4\"\r\n"); // 72
	header.insert(header.size(), "Content-Type: application/octet-stream\r\n\r\n");					 // 42
	header.insert(header.size(), buf2);																 // 1048576
	header.insert(header.size(), "\r\n--myboundary\r\n\r\n");										 // 18
}
