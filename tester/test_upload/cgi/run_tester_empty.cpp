...
void upload_single_file(std::string &header, std::string link)
{
	...
	header.insert(header.size(),
				  "Content-Disposition: form-data; name=\"upload_files[]\"; filename=\"empty\"\r\n");					  // 73
	header.insert(header.size(), "Content-Type: application/octet-stream\r\n\r\n");										  // 42
	header.insert(header.size(), buf);																					  // 0
	header.insert(header.size(), "\r\n--0123456789012345678901234567890123456789012345678901234567890123456789\r\n\r\n"); // 78
}

void upload_multiple_file(std::string &header)
{
	std::ifstream file("/mnt/nfs/homes/gchopin/Documents/webserv/tester/test_upload/files/empty", std::ifstream::in | std::ifstream::binary);
	std::ifstream file2("/mnt/nfs/homes/gchopin/Documents/webserv/tester/test_upload/files/empty", std::ifstream::in | std::ifstream::binary);
	std::istreambuf_iterator<char> it(file), ite;
	...
	header.insert(header.size(),
				  "Content-Disposition: form-data; name=\"upload_files[]\"; filename=\"empty\"\r\n"); // 73
	header.insert(header.size(), "Content-Type: application/octet-stream\r\n\r\n");					  // 42
	header.insert(header.size(), buf2);																  // 0
	header.insert(header.size(), "\r\n--myboundary\r\n\r\n");										  // 18
}
...
int main(int argc, char **argv)
{
	...
	upload_single_all("/mnt/nfs/homes/gchopin/Documents/webserv/tester/test_upload/files/empty", ft_socket);
	ft_close(ft_socket);
	return (0);
}